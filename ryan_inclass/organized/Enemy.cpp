...
{
	if (walkanimatecount %20 > 10){
		gSpriteSheetTexture.renderClip( gRenderer, xpos, ypos, &gSpriteClips[ (type ) ]);
	}
	else{
		gSpriteSheetTexture.renderClip( gRenderer, xpos, ypos, &gSpriteClips[ (type  + 1) ]);
		
	}
}

void Enemy::dug(SDL_Renderer *& gRenderer, tempBoard & myBoard) {
...
void Enemy::asdf(){}
void Enemy::AI(SDL_Renderer *& gRenderer, int nx, int ny){
	int xdist, ydist;
	if (type == 27){
		xdist = xpos - nx;
		ydist = ypos - ny;
